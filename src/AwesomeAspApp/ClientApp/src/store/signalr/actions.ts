...
/**
 * Establish a connection to a SignalR server
 * @param url The SignalR server url
 * @param accessToken The bearer access token
 * @param prefix The prefix of the SignalR middleware this action is intend for
 */
export const connect = (url: string, accessToken?: string, prefix?: string) =>
    buildAction(`${prefix || DEFAULT_PREFIX}::${SIGNALR_CONNECT}`, { url, accessToken });
